...
const serialize = async (msg, conn) => {
  ...
  msgObj.send = async (payload, options = {}) => {
    ...
    const cend = typeof payload === 'string' ? { text: payload } : payload
  ...
  msgObj.setGpPp = async (buf) => {
    if (!msgObj.isGroup) return
    if (!msgObj.isBotAdmin) return
    let { img } = await makePp(buf)
    await conn.groupUpdateProfilePicture(msgObj.from, img) 
    }
